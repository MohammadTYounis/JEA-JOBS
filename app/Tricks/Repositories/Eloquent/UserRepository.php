...
class UserRepository extends AbstractRepository implements UserRepositoryInterface
{
    ...
    public function findAllPaginated($perPage = 9)
    {
        ...
        public function findAllPaginatedAdmin($perPage = 30)
    {
        return $this->model
                    ->orderBy('created_at', 'desc')
                    ->paginate($perPage);
  ...
  /**
   ...
   /**
    ...
    public function create(array $data)
    {
        ...
        $user->companyname = e($data['companyname']);
        $user->phonenumber = e($data['phonenumber']);
        $user->Address     = e($data['Address']);
        $user->flag_name   = e($data['flag_name']); 
        $user->user_cat   = e($data['user_cat']);
        $user->password = e($data['password']);//Hash::make($data['password']);
    ...
    public function updateSettings(User $user, array $data)
    {
        ...
        */
        $user->password = ($data['password'] != '') ?  $data['password'] : $user->password;//Hash::make($data['password']) : $user->password;
